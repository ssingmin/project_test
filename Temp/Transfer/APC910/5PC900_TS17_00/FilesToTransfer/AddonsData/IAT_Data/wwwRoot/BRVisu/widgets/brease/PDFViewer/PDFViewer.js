    ...
    */
    ...
    p._numPadHandler = function (e) {
        ...
        if (this.el && this.elem && this.elem.id === widgetId) {
            ...
            this.elem.setAttribute('data-numpad-shown', '');
        }
    };
    ...
    p.dispose = function () {
        if (this.pdfFrame) {
            this.pdfFrame.get(0).src = '';
            this.el.append(this.pdfFrame);
        }
        this.pdfFrame = null;
        _unbindNumPad(this);
        _hideNumPad.call(this);
    ...
    function _hideNumPad() {
        if (this.elem.hasAttribute('data-numpad-shown')) {
            this.numPad.hide();
            this.elem.removeAttribute('data-numpad-shown');
        }
    }
    ...
    function _pdfOpenParamHandler(widget) {
        ...
        if (i !== undefined) {
            ...
            p.toolbar = parseInt(i.toolbar, 10);
            if (isNaN(p.toolbar)) {
                p.toolbar = (s.showToolbar === true) ? 1 : 0;
            } else {
                p.toolbar = Math.max(0, Math.min(p.toolbar, 1));
            }
        } else {
    ...
    function _pdfOpenParamHandlerIFrame(widget) {
        ...
        widget.pdfUrlOptions = opt + '&pagemode=none';
    }
    ...
    function _includeAlternativeViewer(widget) {
        ...
        widget.el.text('').append(widget.pdfFrame);
    }
